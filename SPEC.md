...
- **Strum audio:** `Tone.Sampler` loading 12 real acoustic-guitar MP3 samples (E2–G4) from `public/samples/guitar-acoustic/`, routed directly to destination (no effects chain — matches the reference library `nbrosowsky/tonejs-instruments`). Samples sourced from `nbrosowsky/tonejs-instruments` (MIT). `baseUrl` uses `import.meta.env.BASE_URL` for GitHub Pages subpath compatibility. `createSynths` awaits `Tone.loaded()` so first strum doesn't fire against empty buffers. Sampler handles polyphony internally and auto-repitches between anchor samples. Audio uses the next unjudged beat's chord (not the playhead chord) to stay in sync with tap matching. **Prior iterations tried and rejected:** `PolySynth(Synth)` with sine oscillator (wrong timbre — original bug); `PolySynth(PluckSynth)` (TypeScript rejects — `PluckSynth` is not `Monophonic`); single `PluckSynth` through EQ3/Chorus/Compressor/Reverb chain (too sharp); 8-voice `PluckSynth` pool (still synthetic). Karplus-Strong cannot match the body resonance of real guitar samples.
- **Strum direction asymmetry (physical-realism tuning):** `playStrum(chord, direction)` differentiates up vs. down on three axes to mimic real guitar technique:
  - **Stagger:** down = 25 ms between notes (low-to-high), up = 10 ms (high-to-low). Up-strums are faster than downstrums, reflecting wrist dynamics.
  - **Velocity:** down = 1.0 (0 dB), up = 0.6 (≈ -4.4 dB). Passed as the 4th argument to `triggerAttackRelease(note, '2n', time, velocity)`.
  - **Bass-string skip on up:** up-strums drop the lowest `Math.min(2, max(0, notes.length - 3))` notes from the voicing before reversing — always leaves ≥3 notes. 4-note voicings (D, D7) drop 1 bass note; 5–6-note voicings (C, Am, A, G, Em, E) drop 2.
...
- **Hook extraction from PracticeScreen:** `useMetronome` (countdown + beat-click RAF, uses `lastClickedBeatRef`), `useDemoMode` (auto-fire beat sounds RAF), `useLoopMode` (overlay state + seamless/overlay restart paths) extracted from `PracticeScreen.tsx` to reduce it from ~625 → ~540 lines and make feature interactions directly testable.
