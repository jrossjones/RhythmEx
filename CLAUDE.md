...
src/
  ...
  components/
    ...
    practice/           # Practice UI components
      ...
      VerticalTimeline.tsx       # Orchestrator for vertical note highway (replaces horizontal BeatTimeline in PracticeScreen); builds current + ghost iterations and loop-seam lines for seamless continuous scroll
      VerticalDrumTimeline.tsx   # N equal-width columns for drum pads, markers scroll downward; renders loop-seam band
      VerticalSingleTimeline.tsx # Single column with horizontal offsets for handpan notes; renders loop-seam band
      VerticalStrumTimeline.tsx  # Single centered column with chord change pill labels for strumming; optional inline scrolling chord diagrams (chordDiagramMode='scroll'); renders loop-seam band
      ChordDiagram.tsx           # SVG beginner chord diagram (6 strings × 4 frets, dots/O/X), used by PracticeScreen and VerticalStrumTimeline
      DebugOverlay.tsx           # Toggleable dev overlay (FPS + Tone.js audio latency + per-tap scheduling offset), gated on settings.debugStatsOn
      ...
      SettingsPopover.tsx # Gear icon popover: metronome, tap sounds, strict mode, speed trainer, loop mode, debug stats toggles
  hooks/
...
- **Outro scroll:** After `elapsed >= durationMs`, `setPhase('done')` fires via `phaseDoneFiredRef` (stops taps). `onDone` callback fires at `durationMs + outroDurationMs` via `onDoneFiredRef`. `useExercise(exercise, onDone, initialBpm?, seamless=false)` — `outroDurationMs = (seamless ? 0 : 1) * msPerBeat(bpm)`: single play and non-seamless loop scroll **one trailing beat** past the hit line before results/restart; seamless loop uses **0** so it wraps exactly at `durationMs`. (Exercises place the last beat one beat before the bar ends, so a seamless wrap leaves exactly one musical beat between the final note and the next round's first note.) `rawProgress` exceeds 1.0 during a non-seamless outro. `PracticeScreen` passes `seamless = settings.loopMode && settings.seamlessLoop`.
...
- **Practice settings:** `PracticeSettings` type with `metronomeOn`, `tapSoundOn`, `strictMode`, `speedTrainerOn`, `speedTrainerStep`, `loopMode`, `seamlessLoop`, `debugStatsOn`. Managed as state in `PracticeScreen`, controlled via `SettingsPopover` gear icon. Settings only changeable in idle phase.
...
- **Loop seam marker:** `loopBoundaryLines` (computed in `VerticalTimeline`, only when `showLoopGhosts`) marks each end→new-start boundary at every integer iteration boundary (`f` from `min(iterationOffsets)` to `max+1`, `yPosition = topPadding + (1 - f) * exercisePixels`). Passed to all three vertical timeline children, each rendering a **translucent red band** (`h-2.5 -translate-y-1/2 bg-red-500/30 z-10`) centered on the boundary; `z-10` keeps it above markers, the 30% opacity lets beats show through. In ghost mode the gray `i=0` divider is skipped so the red seam isn't doubled up. The seam only enters the viewport around each wrap (it marks a point in time, like a measure divider), so it's expected to be off-screen mid-loop.
...
- **Outro scroll timing:** `setPhase('done')` fires at `durationMs` (stops taps via `phaseDoneFiredRef`). `onDone` callback fires later at `durationMs + outroDurationMs` (after beats scroll past — one beat for single/non-seamless, 0 for seamless). Fixes seamless loop phase override — `setPhase('done')` before `onDone` means any phase change inside `handleDone` (e.g., seamless restart → `'playing'`) wins the React batch.
- **Debug stats overlay:** `DebugOverlay` renders only when `settings.debugStatsOn`. Self-contained: measures FPS via its own RAF and reads live Tone.js latency figures from `Tone.getContext()` (`lookAhead`, `rawContext.baseLatency`/`outputLatency`) plus a real per-tap scheduling offset. `useAudio` exposes `audioDebugRef` (`{ tapCount, lastTapPerfMs, lastScheduleAheadMs }`), updated in `playDrum`/`playHandpan`/`playStrum` via `recordTapDebug(scheduledTime)`. Overlay is `pointer-events-none` fixed bottom-left so it never intercepts taps. Added to diagnose reactive-tap latency (default Tone `lookAhead` is 100ms).
